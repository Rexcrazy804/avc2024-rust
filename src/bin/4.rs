...
fn part1<R: BufRead>(reader: R) -> Result<usize> {
    ...
    let the_matrix: Vec<Vec<char>> = reader
        .lines()
        .filter_map(|x| {
            let std::result::Result::Ok(x) = x else { return None }; 
    ...
    for (row_num, rows) in the_matrix.iter().enumerate() {
        for (col_num, &col_char) in rows.iter().enumerate() {
            if col_char == 'X' {
                ...
                if !no_subtract_row && find_xmas(&the_matrix, row_num-1, col_num, String::from('X'), Direction::Up) { answer += 1; }
                if find_xmas(&the_matrix, row_num+1, col_num, String::from('X'), Direction::Down) { answer += 1; }
                if !no_subtract_col && find_xmas(&the_matrix, row_num, col_num-1, String::from('X'), Direction::Left) { answer += 1; }
                if find_xmas(&the_matrix, row_num, col_num+1, String::from('X'), Direction::Right) { answer += 1; }
                if !no_subtract_row && find_xmas(&the_matrix, row_num-1, col_num+1, String::from('X'), Direction::TopRight) { answer += 1; }
                if !no_subtract_row && !no_subtract_col && find_xmas(&the_matrix, row_num-1, col_num-1, String::from('X'), Direction::TopLeft) { answer += 1; }
                if find_xmas(&the_matrix, row_num+1, col_num+1, String::from('X'), Direction::BottomRight) { answer += 1; }
                if !no_subtract_col && find_xmas(&the_matrix, row_num+1, col_num-1, String::from('X'), Direction::BottomLeft) { answer += 1; }
            }
        }
    }
...
fn find_cross_mas(matrix: &Vec<Vec<char>>, row_num: usize, col_num: usize) -> Option<()> {
    if row_num < 1 { return None }
    if col_num < 1 { return None }
    ...
    if ("MAS" == diagonal_one || "SAM" == diagonal_one) && ("MAS" == diagonal_two || "SAM" == diagonal_two) {
        Some(())
    } else {
        None
    }
